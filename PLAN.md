...
_Round 5 (FINAL) — revised after Codex adversarial review_
...
- `export_csv(report, out_path)`: **Gap R1-11:** cells not sanitized — values beginning with `=`, `+`, `-`, `@`, tab, or CR are formula-injection vectors in Excel. Fix: prefix such cells with a single quote or strip the leading character with a warning. **Gap R5-1 (new):** `OUT_FIELDS` preserves only `line/action/sku/original_sku/qty/list/net/extnet/reason` — it omits `extlist`, `dur`, `included`, `sparelist`, and `sparedisc`. R5 says "deal pricing is preserved on every kept line" but the CSV export silently drops these fields. Fix: either add the missing fields to `OUT_FIELDS` or narrow the R5 claim to "list price, net price, extended net, and quantity are preserved; `extlist`, `dur`, `included`, `sparelist`, `sparedisc` are NOT currently exported." **Gap R5-2 (new):** `_num()` silently converts any unparseable or currency-formatted value to `0.0` — a line with a currency string like "$1,200" is read as 0.0, potentially causing R3 to drop a value-bearing bundle. Fix: detect non-numeric cell types explicitly and flag unparseable pricing cells rather than coercing to zero.
...
9. **`keep_zero_dollar_lines` flag** — Steff has not confirmed which behavior is canonical. **Gap R5-3:** this flag changes the actual orderable output — it is not a display preference. Must be resolved before shipping. Either (a) get Steff's answer and hardcode the canonical behavior, or (b) block export if the flag is in its default state and no explicit decision has been recorded.
