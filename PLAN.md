...
_Round 3 — revised after Codex adversarial review_
...
- `_validate(kept, answer_key)`: **Gap R3-1 (new):** extra generated groups outside the answer-key scope pass silently — this undermines the validation claim. Fix: emit a WARN for every kept output group not present in the answer key, so the caller knows the validation is partial. Do not hard-fail (extra groups may be legitimate), but do not silently pass. **Gap R1-10:** maps by line number, silently collapsing duplicate line IDs. Fix: use ordered list or multimap keyed by (line, occurrence index).
...
3. **Descendant classifier** — `CON-*` is kept as primary classifier; unknown non-`CON-*` zero-dollar descendants become UNKNOWN-DESCENDANT flags for human review, not silently kept/dropped.
...
- Pricing field column-parity comparison in `_validate` — R5 pricing preservation is tested by the keep/swap/drop path preserving the price dict; `_validate` is a SKU-correctness diff, not a column audit. Re-raised 3 times without new evidence. Final hold.
