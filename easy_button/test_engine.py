...
# ---------------------------------------------------------------------------
# read_tables block handling
# ---------------------------------------------------------------------------

class TestReadTablesBlocks:
    def _quote_row(self, line_id, sku):
        return _data_row(**{"5": line_id, "7": sku, "14": 100.0})

    def test_three_data_blocks_raises(self):
        rows = (
            [_header_row()]
            + [self._quote_row("1.0", "HW-A")]
            + [_blank_row(), _blank_row()]
            + [self._quote_row("2.0", "HW-B")]
            + [_blank_row(), _blank_row()]
            + [self._quote_row("3.0", "HW-C")]
        )
        sheet = _make_sheet(rows)
        with pytest.raises(ValueError, match="blocks containing"):
            read_tables(sheet)

    def test_footer_third_block_discarded(self):
        rows = (
            [_header_row()]
            + [self._quote_row("1.0", "HW-A")]
            + [_blank_row(), _blank_row()]
            + [self._quote_row("1.0", "SPARE-A")]
            + [_blank_row(), _blank_row()]
            + [_data_row(**{"0": "Notes: contact Steff for questions"})]
        )
        sheet = _make_sheet(rows)
        orig, ak = read_tables(sheet)
        assert len(orig) == 1
        assert len(ak) == 1

    def test_footer_second_block_not_answer_key(self):
        rows = (
            [_header_row()]
            + [self._quote_row("1.0", "HW-A")]
            + [_blank_row(), _blank_row()]
            + [_data_row(**{"0": "Generated by export tool"})]
        )
        sheet = _make_sheet(rows)
        orig, ak = read_tables(sheet)
        assert len(orig) == 1
...
def _minimal_report(has_unknown=False, has_parse_errors=False, has_error=False):
    ...
    sheet = {
        "deal": "TEST",
        "decisions": [decision],
        "has_blocking_flags": has_unknown,
...
class TestExportCsv:
    ...
    def test_failed_validation_blocks(self):
        ...
        report["sheets"][0]["validation"] = {
            "result": "fail", "issues": ["1.1: expected A, got B"], "warnings": []
        }
        with pytest.raises(ValueError, match="FAILED answer-key validation"):
            self._write(report)

    def test_partial_validation_does_not_block(self):
        report = _minimal_report()
        report["sheets"][0]["validation"] = {
            "result": "partial", "issues": [], "warnings": ["group 2 extra"]
        }
